...
async function processJob(env: Env, job: any) {
  ...
  const packages: Array<[RegExp, RegExp, string | undefined]> = [[/^(AnyKernel3|ak3)_.*\.zip$/i, /^(AnyKernel3|ak3)_.*\.zip$/i, isAdmin(env, job.chat_id) ? "构建完成，刷机前请确认机型和序列号。" : undefined]];
  if (String(inputs.nomount_enable).toLowerCase() === "true") packages.push([/^NoMount(?:-Suite)?(?:[-_].*)?(?:\.zip)?$/i, /^NoMount(?:-Suite)?(?:[-_].*)?\.zip$/i, isAdmin(env, job.chat_id) ? "NoMount 模块已随本次构建生成。" : undefined]);
  for (const [artifactPattern, filePattern, caption] of packages) {
    ...
    const outer = new Uint8Array(await download.arrayBuffer()); const unwrapped = unwrapArtifact(outer, filePattern);
